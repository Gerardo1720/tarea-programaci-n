...
public class Estudiante {
    ...
    public double calculorNotaFinal() {
        double promedioParciales = (nota1 + nota2 + nota3) / 3;
        return (promedioParciales * 0.70) + (notaProyecto * 0.30);
    }

    public String Estado() {
        double notaFinal = calculorNotaFinal();
        if (notaFinal >= 90) return "Excelente";
        else if (notaFinal >= 70) return "Aprobado";
        else if (notaFinal >= 60) return "En recuperación";
        else return "Reprobado";
    }

    public String toString() {
        return String.format("%-18s│ Nota final: %5.2f │ %s", nombre, calculorNotaFinal(), Estado());
    }
}